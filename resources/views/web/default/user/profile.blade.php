        ...
        <div class="rounded-lg shadow-sm px-25 py-20 px-lg-50 py-lg-35 position-relative user-profile-info bg-white">
            <div class="profile-info-box d-flex align-items-start justify-content-between">
                ...
                <div class="user-actions d-flex flex-column">
                    <button type="button" id="followToggle" data-user-id="{{ $user['id'] }}" class="btn btn-{{ (!empty($authUserIsFollower) and $authUserIsFollower) ? 'danger' : 'primary' }} btn-sm m-2">
                    ...
                    @php
                        ...
                        @if (!$hideButton)
                            <a class="btn btn-primary {{ (request()->get('tab') == 'appointments' && !request()->get('is_reverse')) ? 'active' : ''  }} btn-sm m-2" id="appointments-tab" href="?tab=appointments#booksession" role="tab">{{ trans('site.book_an_appointment') }}</a>
                        @endif
                    ...
                    @if($user->public_message)
                        ...
                        <button type="button" class="js-send-message btn btn-primary btn-sm m-2 ">{{ trans('site.send_message') }}</button>
                    @endif
                </div>
            </div>
            ...
            <div class="row mt-30 w-100 d-flex align-items-center justify-content-around">
                <div class="col-6 col-md-3  mt-20 mt-md-0 user-profile-state d-flex flex-column align-items-center">
                    ...
                    <span class="font-20 text-dark-blue font-weight-bold mt-5">{{ $totalCount }}</span>
    ...
    <div class="container mt-30 li">
        <section class="rounded-lg border px-10 pb-35 pt-5 position-relative">
            <ul class="nav nav-tabs d-flex align-items-center px-20 px-lg-50 pb-15" id="tabs-tab" role="tablist">
                ...
                {{-- <li class="nav-item mr-20 mr-lg-50 mt-30">
                    <a class="position-relative text-dark-blue font-weight-500 font-16 {{ (request()->get('tab') == 'webinars') ? 'active' : ''  }}" id="webinars-tab" data-toggle="tab" href="#review" role="tab" aria-controls="webinars" aria-selected="false">{{ trans('panel.review') }}</a>
                </li> --}}
                {{-- <li class="nav-item mr-20 mr-lg-50 mt-30">
                ...
                <li class="nav-item mr-20 mr-lg-50 mt-30" id="booksession">
                    <a class="position-relative text-dark-blue font-weight-500 font-16 {{ (request()->get('tab') == 'appointments' && !request()->get('is_reverse')) ? 'active' : ''  }}" id="appointments-tab" href="?tab=appointments
                    " role="tab" >{{ trans('site.book_an_appointment') }}</a>
                </li>

                {{-- <li class="nav-item mr-20 mr-lg-50 mt-30">

                    <a class="position-relative text-dark-blue font-weight-500 font-16 {{ (request()->get('tab') == 'appointments' && request()->get('is_reverse')) ? 'active' : ''  }}" id="appointments-tab" href="?tab=appointments&is_reverse=1
                    " role="tab">{{ trans('site.book_an_appointment_reverse') }}</a>
                </li> --}}
            </ul>
            ...
            <div class="tab-content" id="nav-tabContent">
                ...
                {{-- <div class="tab-pane fade px-20 px-lg-50 {{ (empty(request()->get('tab')) or request()->get('tab') == 'about') ? 'show active' : ''  }}" id="review" role="tabpanel" aria-labelledby="about-tab">
                    @include('web.default.user.profile_tabs.review')
                </div> --}}
                ...
                @if(request()->get('tab') == 'appointments')
                    @if(request()->get('is_reverse'))
                        @php
                            $is_reverse = true;
                        @endphp
                    @endif
