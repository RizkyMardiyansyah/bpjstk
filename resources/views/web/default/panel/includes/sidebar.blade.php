<div class="xs-panel-nav d-flex d-lg-none justify-content-between py-5 px-15">
    ...
    <button style="width: 100%" class="btn btn-primary btn-transparent d-flex flex-column-reverse justify-content-center align-items-center p-5 rounded-sm sidebarNavToggle" type="button">
...
<div class="panel-sidebar pt-50 pb-25 px-25" id="panelSidebar">
    ...
    <ul id="panel-sidebar-scroll" class="sidebar-menu pt-10 @if(!empty($authUser->userGroup)) has-user-group @endif" @if((!empty($isRtl) and $isRtl)) data-simplebar-direction="rtl" @endif>
        ...
        <li class="sidenav-item {{ (request()->is('panel/diary') or request()->is('panel/diary/*')) ? 'sidenav-item-active' : '' }}">
            <a class="d-flex align-items-center" data-toggle="collapse" href="#diaryCollapse" role="button" aria-expanded="false" aria-controls="diaryCollapse">
                <span class="sidenav-item-icon mr-10">
                    @include('web.default.panel.includes.sidebar_icons.diary')
                </span>
                <span class="font-14 text-dark-blue font-weight-500">{{ __('diary.diary') }}</span>
            </a>

            <div class="collapse {{ (request()->is('panel/diary') or request()->is('panel/diary/*')) ? 'show' : '' }}" id="diaryCollapse">
                <ul class="sidenav-item-collapse">
                    <li class="mt-5 {{ (request()->is('panel/diary/create')) ? 'active' : '' }}">
                        <a href="/panel/diary/create">{{ trans('diary.create_diary') }}</a>
                    </li>
                    <li class="mt-5 {{ (request()->is('panel/diary')) ? 'active' : '' }}">
                        <a href="/panel/diary">{{ trans('diary.list') }}</a>
                    </li>
                </ul>
            </div>
        </li>
            ...
            <div class="collapse {{ (request()->is('panel/meetings') or request()->is('panel/meetings/*')) ? 'show' : '' }}" id="meetingCollapse">
                <ul class="sidenav-item-collapse">
                    ...
                    @if($authUser->isTeacher())
                        @php
                            $meetingIds = App\Models\Meeting::where('creator_id', $authUser->id)->pluck('id');
                            $meetingHours = App\Models\ReserveMeeting::whereIn('meeting_id', $meetingIds)                                            
                                            ->pluck('meeting_time_id');
                        
                            $activeMeetingTimes = App\Models\MeetingTime::whereIn('id', $meetingHours)->get();
                            $timeTable = array_count_values($meetingHours->toArray());
                            $activeHoursCount = 0;
                            ...
                            // Hitung dan bulatkan hasil
                            $activeHoursCount = floor($activeHoursCount / 3600);
                        @endphp
