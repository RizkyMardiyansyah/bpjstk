...
class InstructorFinderController extends Controller
{
    ...
    public function index(Request $request)
    {
                $query = User::where('users.status', 'active')
                    ->where('users.role_name', 'teacher') // Hanya ambil pengguna dengan peran 'teacher'
    ...
    private function handleFilters(\Illuminate\Database\Eloquent\Builder $query, Request $request)
    {
        $categoryId = $request->get('category_id', null);
        $levelOfTraining = $request->get('level_of_training', null);
        $typeOfSessions = $request->get('type_of_sessions', null);
        ...
        if(!empty($typeOfSessions) and is_array($typeOfSessions)){
            // Careful of Operator Precedence
            $query->where(function ($query) use ($typeOfSessions) {
                foreach ($typeOfSessions as $typeOfSession) {
                    $query->orWhere('type_of_sessions', "like", "%$typeOfSession%"); // "like" is case insensitive
                }
            });
        }
    ...
    public function wizard(Request $request)
    {
        ...
        if ($step > 4) {
            ...
            $url = '/contributors?' . http_build_query($params);
